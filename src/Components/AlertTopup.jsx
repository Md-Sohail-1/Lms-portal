...
const AlertTopup = ({message, link, display}) => {
  ...
  const [alertDisplay, setAlertDisplay] = useState(display);
  ...
  */
  ...
  return (
    <div className={`${display ? "flex" : "hidden"} h-screen w-screen fixed z-10 flex items-center justify-center`} >
      <div className={`flex flex-col gap-2 bg-slate-200 items-center justify-center gap-6 min-h-48 md:w-1/3 w-2/3 rounded-md border-2 border-violet-500`} >
        ...
        <div className="flex gap-2" >
          <button onClick={()=>setCancel(!cancel)} className="bg-white hover:bg-gray-100 active:bg-gray-300 px-6 py-1 rounded-md text-black shadow-md font-bold" >Cancel</button>
